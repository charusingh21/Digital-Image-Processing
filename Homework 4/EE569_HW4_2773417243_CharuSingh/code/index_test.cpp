...
void load_image(string filename, int tensorSet[][5][5]){
    ...
    //Writing to a file
    ofstream myfile("result_test.txt", ios::app);
    for(int i=0; i< 15; i++){
        myfile << finalArr[i] << " ";
    }
    myfile << "\n";
...
main(){
    ...
    const char *files[12] = {"10.raw", "11.raw", "12.raw", "1.raw", "2.raw", "3.raw", "4.raw", "5.raw", "6.raw", "7.raw", "8.raw", "9.raw"};
    const string defaultPath = "/media/sf_SharedFolder/Charu/HW4/images/";
    ofstream myfile("result_test.txt");
    myfile.open("result_test.txt", ios::trunc);
    myfile.close();
    for(int i=0; i<12; i++){
        string s;
        s+=files[i];
