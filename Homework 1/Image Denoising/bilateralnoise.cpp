...
vector<vector<double>>  getMaskedMatrix(unsigned char bufferImage[][320],int i,int j){        //get bilateral kernel
    float sigma1 = 1;
    float sigma2 = 1;
    ...
    vector<vector<double>> maskMatrix(5,vector<double>(5));
     for(int k=i-2 ; k< i+3; k++){
            for(int l = j-2; l< j+3 ; l++){
                W = exp(-((pow((k-i),2) + pow((l-j), 2))/(2*sigma1*sigma1))- (pow((bufferImage[i][j]-bufferImage[k][l]),2)/(2*sigma2*sigma2)));

             //   W = exp(-(((pow((k-i),2) + pow((l-j), 2))/(2*sigma1*sigma1))+ (pow(i,2)+pow(bufferImage[k][l],2),0.5)/(2*sigma2*sigma2))));)
               maskMatrix[k-i+2][l-j+2] = W;  
            }
     }
     return maskMatrix;
}
