...
//	int BytesPerPixel=3;
	...
	if (!(file=fopen("images/gearoutputthin.raw","rb"))) {
        cout << "ooopsie!!!!";
	...
	fread(Imagedata, sizeof(unsigned char), X*Y, file);
     ...
     if (!(file=fopen("gearfillholes.raw","wb"))) {
		cout << "ooopsie!!!!";
		exit(1);
	}
    ...
    int  count=0;
     for (int j = 125; j < 249; j++)
    {
        
            if (  Imagedata_normalized[124][j]==1){
